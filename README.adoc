...
Instantiating container, mounting the parent directory (assuming that contains all openEHR specifications repos):
[source]
----
cd ..
docker run --rm -it -u $(id -u):$(id -g) -v "$(pwd):/documents/" --entrypoint bash openehr/asciidoctor
----
...
Running asciidoctor as a tool (considering that the docker CMD is defined `spec_publish.sh -f -r -v -t -q -l`):
[source]
----
